...
export const ModelsForm = () => {
    ...
    const submit = (data) => {
        ...
        formData.append('name', data.name_en);  // Matn ma'lumotlarini qo'shyapmiz
        formData.append('title', data.name_ru);
    ...
    return (
        <div className='flex items-center justify-center py-12 bg-slate-500'>
            <form className='w-[500px] h-[600px] border-2  flex flex-col gap-5 px-20 py-40 rounded-[30px] bg-white' onSubmit={handleSubmit(submit)} encType="multipart/form-data">
                <input  className='w-[350px] py-4 outline-bground  rounded-xl px-3 border-bground border'  {...register("name")} type="text" required placeholder='car name' />
                <input  className='w-[350px] py-4 outline-bground rounded-xl px-3  border-bground border' {...register("title")} type="text" required placeholder='title' />
